				...
				<div className='studyContainer'>
						...
						<div className='quizHeader'>
							<div className='quizTitle'>
								<span>{quiz.get('title')} тест</span>
								<span style={{ marginLeft: 10 }}></span>
							</div>
							{!this.state.complete
								? <h3 className='quizMeta'>Вопрос {index + 1} из {numberOfQuestions}</h3>
								: <h3 className='quizMeta'>Тест завершён</h3>}
								{isDesktop &&
									<span id="return">
										<Link to='/'>
					...
					{answer !== null && !complete &&
						<div className='messageDiv'>
							{answer
								? <h1 className='correctAnswer'>Правильный ответ!</h1>
								: <h1 className='wrongAnswer'>Ответ неправильный!</h1>}
							...
							{index + 1 === numberOfQuestions
								? <button onClick={this.nextQuestion}>Смотреть результаты</button>
								: <button onClick={this.nextQuestion}>Следующий вопрос</button>}
						</div>}

					{complete &&
						<div>
							<h1 className='scoreMessage'>
								Вы правильно ответили на {score} вопроса(ов) из {numberOfQuestions}. { percentage > 0.75 ? 'Отличная работа!' : 'Попробуйте в другой раз!'}
							</h1>
						...
						{!isMobile && <div id='infoBox'>
							<p>Можете использовать кнопки: <i className='fa fa-long-arrow-up'></i>, <i className='fa fa-long-arrow-down'></i>, пробел и esc</p>
						</div>}
